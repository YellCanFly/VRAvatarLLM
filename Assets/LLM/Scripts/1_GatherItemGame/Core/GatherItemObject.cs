...
public class GatherItemObject : MonoBehaviour
{
    ...
    public GatherItemObjectInfoInt GetGrabItemInfoInt()
    {
        ...
        result.height = Mathf.RoundToInt(interactObject.transform.position.y * 100);


        var rel_pos_to_user = interactObject.GetRelativePositionToCamera(GatherItemManager.Instance.gazeSphereDetector.transform);
        result.obj_rel_user = new int[]
        {
            Mathf.RoundToInt(rel_pos_to_user.x * 100),
            Mathf.RoundToInt(rel_pos_to_user.z * 100)
        };
        var rel_pos_to_avatar = interactObject.GetRelativePositionToCamera(GatherItemManager.Instance.avatarAcitivate.transform);
        result.obj_rel_avatar = new int[]
        {
            Mathf.RoundToInt(rel_pos_to_avatar.x * 100),
            Mathf.RoundToInt(rel_pos_to_avatar.z * 100)
        };
        var obj_size_m = interactObject.GetObjectBounds().size;
        result.scale = Mathf.RoundToInt((obj_size_m.x + obj_size_m.y + obj_size_m.z) * 33); // Scale is the max size in cm
...
public class GatherItemObjectInfoInt
{
    ...
    [JsonProperty("obj_rel_user")]
    public int[] obj_rel_user;

    [JsonProperty("obj_rel_avatar")]
    public int[] obj_rel_avatar;
}
