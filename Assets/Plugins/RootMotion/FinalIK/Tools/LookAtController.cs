...
namespace RootMotion.FinalIK {
	...
	public class LookAtController : MonoBehaviour {
		...
		private AimController AimController; // Added
		private int LeftHash = Animator.StringToHash("Left"); // Added
		private int RightHash = Animator.StringToHash("Right"); // Added
        ...
        private void RootRotation() 
		{
			if (target == null || AimController.target != null) return;
			...
			if (max < 180f) {
				...
				rotation *= 0.01f; // small adjustment
				ik.transform.rotation = Quaternion.AngleAxis(rotation, ik.transform.up) * ik.transform.rotation;
			}
		}
	}
